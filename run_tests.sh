...
function run_tests {
  ...
  if [ $debug -eq 1 ]; then
    if [ "$testropts" = "" ] && [ "$testrargs" = "" ]; then
      ...
      # provided.
      testrargs="discover ./pcsnovadriver/tests"
    fi
  ...
  if [ setup.cfg -nt pcs-nova-driver.egg-info/entry_points.txt ]
  then
  ...
  if [ $coverage -eq 1 ]; then
    ...
    ${wrapper} coverage html --include='pcsnovadriver/*' --omit='nova/openstack/common/*' -d covhtml -i
  fi
